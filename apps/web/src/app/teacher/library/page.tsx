...
export default function TeacherLibraryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) =>
      ...
      (r.created_by_name ?? "").toLowerCase().includes(needle)
    );
  }, [rows, q]);
    ...
    <div className="flex flex-1 flex-col gap-4 p-4 lg:p-6">
      ...
      {copiedQuiz && (
        <div role="status" className="rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-700">
          Kuis tersalin ke Daftar Soal kamu dengan kode baru: <span className="font-mono font-bold">{copiedQuiz}</span> — original mengacu ke pembuat asli.
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        <div className="relative w-full sm:max-w-sm">
          ...
          <Input placeholder="Cari judul / kode / mapel / pengarang..." value={q} onChange={(e) => setQ(e.target.value)} className="pl-8" />
        </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {filtered.map((r) => {
            ...
            const isCopy = !!r.original_by && r.original_by !== r.created_by;
            return (
              <div key={r.id} className="flex flex-col overflow-hidden rounded-lg border border-neutral-200 bg-white">
                ...
                <div className="flex flex-1 flex-col gap-2 px-3 py-3">
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    {isMine ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-medium text-blue-700">Milikku</span>
                    ) : (
                      <span className="inline-flex items-center gap-1 rounded-full bg-neutral-100 px-2 py-0.5 text-[11px] font-medium text-neutral-600">oleh {r.created_by_name || "guru lain"}</span>
                    )}
                  </div>
                  {isCopy && r.original_by_name && (
                    <span className="inline-flex items-center gap-1 text-[11px] text-neutral-500">
                      <HugeiconsIcon icon={Copy02Icon} size={12} /> Original by {r.original_by_name}
                    </span>
                  )}
                  ...
                  <div className="mt-auto flex gap-2 pt-2">
                    <Link href={`/teacher/library/${encodeURIComponent(r.code)}`} className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-input bg-background px-2.5 py-1.5 text-xs font-medium hover:bg-muted">
                      Lihat detail <HugeiconsIcon icon={ArrowRight01Icon} size={12} />
                    </Link>
                    {isMine ? (
                      <Link href={`/teacher/quizzes/${encodeURIComponent(r.code)}`} className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-secondary px-2.5 py-1.5 text-xs font-medium text-secondary-foreground hover:bg-secondary/80">
                        <HugeiconsIcon icon={Idea01Icon} size={12} /> Edit
                      </Link>
                    ) : (
                      <button type="button" onClick={() => handleCopy(r.code)} className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-secondary px-2.5 py-1.5 text-xs font-medium text-secondary-foreground hover:bg-secondary/80">
                        <HugeiconsIcon icon={Copy02Icon} size={12} /> Salin
                      </button>
                    )}
                  </div>
                </div>
