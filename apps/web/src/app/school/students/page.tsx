...
export default function StudentsPage() {
  ...
  async function load() {
    const res = await fetch(`${apiUrl}/school/students`, { headers: { Authorization: `Bearer ${getToken()}` } });
  ...
  async function create() {
    setLoading(true);
    const res = await fetch(`${apiUrl}/school/students`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` }, body: JSON.stringify(form) });
    const data = await res.json();
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 lg:p-6">
      ...
      <Button onClick={create} disabled={loading} className="w-fit">{loading ? "..." : "Tambah Murid"}</Button>
